        ...
        $(document).on("click", "#add_more", function (e) {
            ...
            $.ajax({
                ...
                success: function (msg) {
                    ...
                    html += '<div class="col-sm-12  col-md-2   form-group pull-left">';
                    html += '<label class="control-label">' + lang.meals + '</label>';
                    html += '<div class="selector"><select name="meals[]" class="full-width required_field">';
                    for (var i in result.meals) {
                        html += '<option value="' + i + '">' + result.meals[i] + '</option>';
                    }
                    var first_meal = result.meals[Object.keys(result.meals)[0]]; //returns first element in array
                    html += '</select><span class="custom-select full-width">' + first_meal + '</span></div></div>';

                    html += '<div class="col-md-6">';
                    html += '<div class="col-sm-12  col-md-3 form-group pull-left">';
                    html += '<label class="control-label">' + lang.rooms + '</label>';
                    ...
                    html += '<input type="number" value="0"  name="children[]" class="input-text full-width"></div>';
                    html += '<div class="col-sm-12  col-md-3   form-group pull-left">';
                    html += '<label class="control-label">' + lang.infants + '</label>';
                    html += '<input type="number" value="0" name="infants[]" class="input-text full-width"></div>';

                    html += '</div>';
                    html += '<div class="col-sm-1 form-group pull-left">';
